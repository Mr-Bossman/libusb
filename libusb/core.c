 ...
 */
  ...
  * - libusb_get_blockdev_path()
  * - libusb_get_bos_descriptor()
  * - libusb_get_bus_number()
  * - libusb_get_chardev_path()
 ...
 * \param path output buffer for the USB block device path
 * \param path_length size of path buffer
 * \returns \ref LIBUSB_ERROR_NOT_FOUND the device doesn't have an associated device
 * \returns another LIBUSB_ERROR code on error
 */
int API_EXPORTED libusb_get_blockdev_path(libusb_device *dev, int iface_idx,
	char *path, size_t path_length)
{
	...
	return usbi_backend.get_dev_path(dev, iface_idx, USBI_DEV_BLOCK, path, path_length);
}
 ...
 * \param dev a device
 * \param iface_idx the <tt>bInterfaceNumber</tt> of the interface you wish to probe
 * \param path output buffer for the USB character device path
 * \param path_length size of path buffer
 * \returns \ref LIBUSB_ERROR_NOT_FOUND the device doesn't have an associated device
 * \returns another LIBUSB_ERROR code on error
 */
int API_EXPORTED libusb_get_chardev_path(libusb_device *dev, int iface_idx,
	char *path, size_t path_length)
{
	struct libusb_config_descriptor *config;
	int r;
	...
	return usbi_backend.get_dev_path(dev, iface_idx, USBI_DEV_CHAR, path, path_length);
}
